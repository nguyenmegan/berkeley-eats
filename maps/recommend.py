...
def group_by_centroid(restaurants, centroids):
    ...
    for each_restaurant in restaurants:
        res_location = restaurant_location(each_restaurant)
        closest_centroid = find_closest(res_location, centroids)
        lst += [[centroids.index(closest_centroid), each_restaurant]]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b = S_xy/S_xx
    a, r_squared = meany - b * meanx, S_xy ** 2/(S_xx * S_yy)
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    lst = max([find_predictor(user, reviewed, f) for f in feature_fns], key = lambda x: x[1])
    return lst[0]
    # END Question 8
